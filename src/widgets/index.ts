...
const WIDGET_DOMAIN = process.env['PUBLIC_BASE_URL'] ?? 'https://nexus.agentixpay.ai';
...
export function widgetMeta(): Record<string, unknown> {
  return {
    ui: {
      ...
      csp: {
        connectDomains: [process.env['NEXUS_BASE_URL'] ?? 'https://nexus-api.agentixpay.ai'],
...
const IMG_PROXY_ORIGIN = process.env['PUBLIC_BASE_URL'] ?? 'https://nexus.agentixpay.ai';
...
//
// (Tried calling window.openai.callTool directly and rendering the result
// in-place instead — reverted. It only works for buttons that stay within
// the SAME widget template (merchant → categories → products, which all
// share detection logic in one render()). get_product / add_to_cart /
// create_handoff each render through a DIFFERENT widget (product-detail /
// cart-view / checkout-link) — forcing their result through the current
// widget's own render(), which doesn't know those shapes, fell through to
// the merchants-empty branch and showed "No stores available" on every
// Buy/Details/Add-to-cart click. sendFollowUpMessage lets the host mount
// the correct widget for the target tool instead.)
